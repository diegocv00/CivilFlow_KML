              ...
              return tramosSan.map(t=>{
                ...
                return(
                  <tr key={t.id}>
                    ...
                    <td className="c"><input type="text" inputMode="decimal" className="ni" style={{width:70,padding:'2px 4px',fontSize:12,textAlign:'center'}} defaultValue={t.nmaning!==undefined?String(t.nmaning):'0'} onBlur={e=>{const raw=e.target.value.replace(/,/g,'.');const v=parseFloat(raw);if(!isNaN(v))updTramoSan(t.id,'nmaning',v);}}/></td>
                    <td className="c"><input type="number" className="ni" style={{width:36,padding:'2px 3px',fontSize:10,textAlign:'center'}} value={sVal} step="0.1" min={0} onChange={e=>updTramoSan(t.id,'sPercent',parseFloat(e.target.value)||2)}/></td>
